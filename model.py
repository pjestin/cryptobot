...
class Kline:
    ...
    KLINE_LABELS = ['open_time', 'open_price', 'high_price', 'low_price', 'close_price', 'volume', 'close_time', 'quote_asset_volume', 'nb_trades']

    def __init__(self, kline_data):
        for k in range(len(self.KLINE_LABELS)):
            setattr(self, self.KLINE_LABELS[k], float(kline_data[k]) if isinstance(kline_data[k], str) else kline_data[k])
